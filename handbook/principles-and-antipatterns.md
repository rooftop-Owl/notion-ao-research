...
The 30-second test is brutal and useful. Give a page to someone unfamiliar. If they cannot identify input location in 30 seconds, your design failed. Don’t explain it away. Redesign it.
...
1. **Navigation buttons (네비게이션 버튼 페이지)**  
   Pages that exist primarily as click targets to move elsewhere.
2. **Master tables (마스터 테이블)**  
   Core source-of-truth databases that must be quickly distinguishable.
3. **Category/relation pages (카테고리/관계형 페이지)**  
   Pages that represent classification nodes where visual grouping improves recognition.
4. **External-facing pages (외부 공개형 페이지)**  
   Marketing/recruiting/showcase pages where presentation is a first-class objective.
...
### 1) Master table names in English (마스터 테이블 영문 네이밍)

Keep master table names in English and keep them stable. This is less about language preference and more about consistent indexing and cross-page readability. Choose one naming style and keep it:
- `Tasks`, `Projects`, `Meetings`, `References`, `Contacts`

Do not rename tables every month. Stability is more valuable than novelty.

### 2) `입력완료 ✓ (Input Complete ✓)` trigger pattern

Use a clear trigger field name such as `입력완료 ✓` to communicate lifecycle intent. The checkmark is not decoration; it is a workflow contract. Everyone understands that checking this field moves a record from input to processing/completed contexts.
...
### 3) Yellow background convention (노란 배경 규칙)
...
### 4) Lifecycle-order status options (수명주기 순서 상태값)

Status options must follow process order, not emotional order or random preference.

Example lifecycle sequence:
- `Inbox` → `Input Complete` → `In Progress` → `Complete` → `Archived`

Avoid unordered status lists like `Urgent`, `Later`, `Maybe`, `Final`, `Start`. Those are labels, not lifecycle states. A status field should encode transition logic.

## Anti-Patterns

These are the recurring mistakes that make workspaces look active while quietly killing usage.
...
| Icons on every page | Visual noise hides content status and slows scanning | Apply the 4-criteria icon rule |
...
A quick diagnostic: if your team keeps asking “Where do I write this?” or “Why did this disappear?” you are likely running at least three anti-patterns simultaneously.
...
If your workspace follows the 3 Principles and avoids these anti-patterns, it will outlast any template you download.
